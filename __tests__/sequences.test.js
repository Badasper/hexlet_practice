...
describe('First N elements of sequence', () => {
  test('set 1', () => {
    expect(listToString(sequencesTask.firstNElements(3, l()))).toBe(listToString(l()));
  });

  test('set 2', () => {
    expect(listToString(sequencesTask.firstNElements(3, l(1, 2)))).toBe(listToString(l(1, 2)));
  });

  test('set 3', () => {
    expect(listToString(sequencesTask.firstNElements(1, l(1, 2)))).toBe(listToString(l(1)));
  });
});
...
describe('Zip', () => {
  it('set 1', () => {
    ...
    const result = sequencesTask.zip(list1, list2);
...
describe('Same Parity as first eleme', () => {
  test('SameParity', () => {
    const result = sequencesTask.sameParity(l(5, 0, 1, -3, 10));
    expect(listToString(result)).toBe(listToString(l(5, 1, -3)));

    const result2 = sequencesTask.sameParity(l(2, 0, 1, -3, 10, -2));
    expect(listToString(result2)).toBe(listToString(l(2, 0, 10, -2)));

    const result3 = sequencesTask.sameParity(l(-1, 0, 1, -3, 10, -2));
    expect(listToString(result3)).toBe(listToString(l(-1, 1, -3)));

    const result4 = sequencesTask.sameParity(l());
...
describe('Is Safe Queens', () => {
  it('safe', () => {
    ...
    expect(sequencesTask.isSafeQueens(queens1)).toBe(true);

    const queens2 = l(3, 6, 2, 5, 1, 4);
    expect(sequencesTask.isSafeQueens(queens2)).toBe(true);

    const queens3 = l(1, 5, 8, 6, 3, 7, 2, 4);
    expect(sequencesTask.isSafeQueens(queens3)).toBe(true);
  });

  it('not safe', () => {
    const queens1 = l(2, 1, 3);
    expect(sequencesTask.isSafeQueens(queens1)).toBe(false);

    const queens2 = l(1, 2, 3, 4, 5, 6);
    expect(sequencesTask.isSafeQueens(queens2)).toBe(false);

    const queens3 = l(1, 5, 8, 6, 3, 7, 2, 8);
    expect(sequencesTask.isSafeQueens(queens3)).toBe(false);

    const queens4 = l(3, 3, 3, 3, 3, 3);
    expect(sequencesTask.isSafeQueens(queens4)).toBe(false);
  });
});
...
describe('Union', () => {
  it('set 1', () => {
    ...
    const result = sequencesTask.union(list1, list2);
    expect(listToString(result)).toBe('()');
  });
...
describe('Flatten', () => {
  it('set 1', () => {
    ...
    expect(listToString(sequencesTask.flatten(list))).toBe('()');
  });

  it('set 2', () => {
    const list = l(1, 2, l(3, 5), l(l(4, 3), 2));

    expect(listToString(sequencesTask.flatten(list))).toBe('(1, 2, 3, 5, 4, 3, 2)');
  });

  it('set 3', () => {
    const list = l(l(1, l(5), l(), l(l(-3, 'hi'))), 'string', 10, l(l(l(5))));

    expect(listToString(sequencesTask.flatten(list))).toBe('(1, 5, -3, hi, string, 10, 5)');
  });
});
...
describe('Sort', () => {
  it('set 1', () => {
    const result = sequencesTask.sort(l());
    expect(listToString(result)).toBe(listToString(l()));

    const result2 = sequencesTask.sort(l(5, -3, 2, 10, 4, 4, 5));
    expect(listToString(result2)).toBe(listToString(l(-3, 2, 4, 4, 5, 5, 10)));

    const result3 = sequencesTask.sort(l(3, 3, 0, -1, 0, 4, -5));
